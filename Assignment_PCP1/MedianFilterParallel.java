//Assignment_PCP1:  Parallel Median Filter
...
public class MedianFilterParallel {
    ...
    static int[] median(int[] pixels, int width){
        return fjPool.invoke(new MedianArray(pixels, 0, pixels.length, width));
    }

    static int[] median_filter(int[] p_arr, int w_width){
        //initialise an integer array to store the new pixel values 
        int[] new_pixels = new int[p_arr.length];

        tic(); //start timer
        new_pixels = median(p_arr, w_width);
        toc(); //end time and calculate runtime

        //display timing message
        System.out.println("Runtime for parallel median filter with window of size " + Integer.toString(w_width) + "x" + Integer.toString(w_width) 
    ...
    public static void main(String args[])throws IOException{
        ...
        int[] median_pixels = new int[(img_height - border_width)*(img_width - border_width)];
        median_pixels = median_filter(pixels, window_width);

        count = 0;
        //set the pixel value to the median of the RGB values
        for (int i = border_width; i < (img_height - border_width); i++){
            for (int j = border_width; j < (img_width - border_width); j++){
                img.setRGB(j, i, median_pixels[count]);
                count += 1;
            } //image width loop ends here
        }//image height loop ends here
